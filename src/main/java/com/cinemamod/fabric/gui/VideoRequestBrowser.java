...
public class VideoRequestBrowser extends Screen {
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        if (!urlField.isFocused()) {
            ...
            urlField.setCursor(0, client.options.sneakKey.isPressed()); // If the URL is longer than the URL field, we want it to start at the beginning
        }
    ...
    public boolean mouseScrolled(double mouseX, double mouseY, double deltaX, double deltaY) {
        ...
        if (urlField.isFocused()) {
            urlField.mouseScrolled(mouseX, mouseY, deltaX, deltaY);
        }
        ...
        browser.sendMouseWheel(mx(mouseX), my(mouseY), 0, (int)deltaY, 90);
